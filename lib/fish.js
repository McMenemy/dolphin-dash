...
(function () {
  ...
  var FISH_TO_PLAYER_DETECTION_RADIUS = 100;

  var Fish = MicroMunch.Fish = function (options) {
  ...
  MicroMunch.Util.inherits(Fish, MicroMunch.MovingObject);
  ...
  Fish.prototype.isNearOtherFish = function (otherFish) {
    return MicroMunch.Util.distance(this.pos, otherFish.pos) < FISH_TO_FIST_DETECTION_RADIUS;
  };

  Fish.prototype.isNearPlayer = function (player) {
    return MicroMunch.Util.distance(this.pos, player.pos) < FISH_TO_PLAYER_DETECTION_RADIUS;
  };
  ...
  Fish.prototype.isEaten = function (player, dolphRadius) {
