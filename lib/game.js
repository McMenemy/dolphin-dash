...
(function () {
  ...
  Game.prototype.addFish = function () {
    ...
    for (var r = 0; r < FISH_ROWS; r++) {
      ...
      for (var c = 0; c < FISH_COLS; c++) {
        yCoord += 25;
        var newFish = new MicroMunch.Fish({ pos: [xCoord, yCoord] });
        fish.push(newFish);
      }
    }
  ...
  Game.prototype.schoolFish = function () {
    ...
    for (var f1 = 0; f1 < this.fish.length; f1++) {
      ...
      if (fish.isEaten(player, player.radius)) {
        eatenFish.push(f1);
      }

      if (fish.isNearPlayer(player)) {
        fish.moveAwayFromPredator(player);
      }

      if (fish.isNearPlayer(predator)) {
