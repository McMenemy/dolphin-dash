(function () {
  ...
  Fish.prototype.isDolphinMovingToward = function (dolphin) {
    var posDiffPresent = DolphinDash.Util.distance(this.pos, dolphin.pos);
    var posFuture = [dolphin.pos[0] + dolphin.vel[0], dolphin.pos[1] + dolphin.vel[1]];
    var posDiffFuture = DolphinDash.Util.distance(posFuture, dolphin.pos);

    return (posDiffPresent - posDiffFuture) < 0;
  };

  Fish.prototype.moveAwayFromDolphin = function (dolphin) {
    

    var unitNormalVector = DolphinDash.Util.calcAwayUnitNormalVector(this.vec, this.pos, dolphin.vec, dolphin.pos)

    this.vel = [dolphin.vel[0] * 1.2, dolphin.vel[1] * 1.2];
  };
